...
## 2026-07-30 — iOS 登录资料与报告上传主体同步

- 修复“有效登录却在上传报告前提示当前登录信息不完整”：旧实现只保存 token，`/api/users/me` 结果又停留在首页、设置或家庭页面的局部状态；报告上传只读取易丢失的内存 `userInfo.id`，忽略后端 JWT 已签名的数字 `sub`。
- `AuthManager` 现在以当前有效 JWT 的正整数 `sub` 作为账号级写操作主体，`userInfo.id` 和旧 `subjectId` 仅作兼容回退；手机号登录和受试者登录成功后主动同步 `/api/users/me`，其余资料读取也统一通过账号作用域校验后回写全局状态。
- 旧账号迟到响应、响应 ID 与当前 JWT 不一致、非数字或非正数主体均被拒绝；同一账号正常刷新 token 时仍可采用请求期间同账号作用域的资料。
- 与远端 main 合并后的最终回归：登录同步与身份解析 `29/29`、报告相邻逻辑 `12/12`、报告上传完整界面 `1/1` 均通过 `.xcresult` 校验；界面确定性网络审计为零逃逸，校验器自测 `8/8`，regression guard、fast 与 impacted 轻量门禁通过。
- 推送前发现远端新增 8 个 Unit、1 个 UI 测试及画像/评分/合规组件；已保留远端实现并把精确清单更新为 Unit `205`、主 UI `9`、小屏 UI `2`、总集 `214`。Swift 聚合预算只同步到当前实际 `10032` 行、`106` 个 struct，增加一行或一个 struct 的负向回归仍失败。
- 本轮仅修改 iOS，未修改 Android。真实生产 `/api/users/me`、对象存储和 OCR 可用性仍需受控环境或 TestFlight 验证；发布状态以本日志后续 TestFlight 回执为准。

---

## 2026-07-27 — 健康画像、合规页面与首页三评分完善

> 交付范围：PR [#5](https://github.com/LoveWood233/XJie_IOS/pull/5) 合入 `origin/main` 后的健康画像、体重、账号/合规与首页三评分体验改进；后续小修直接进入 `main`。本节仅记录已合入提交和实际执行的验证，不将轻量门禁描述为真机或 TestFlight 验收。

### 健康画像、体重与账号/合规

- `295f549`（经 PR #5 合入，合并提交 `b2874e8`）完善健康画像二级编辑页：基础资料支持出生日期、性别、血型、省市和数字键盘身高录入；移除体重项与无实际作用的“填写内容”控件；长期健康标签和安全信息保持卡片原位展开编辑。
- 画像页面在读取失败或请求被拦截时仍保留可调试布局和编辑入口；长期用药、健康目标与计划继续跳转至各自的唯一业务数据源。
- 体重详情与趋势视图优化：最近记录、BMI/身高补录、近三个月趋势、纵轴刻度、横向查看和长按选点维持在 `XAgeMetricTrendView.swift`，不新增独立本地数据源。
- 更多菜单与设置页补充账号与安全、隐私政策、权限申请与使用说明、反馈输入样式及预览支持；隐私和权限正文同步维护在 `docs/legal/`。
- 新增 `update_upstream_main.command`，用于从项目目录快速拉取并快进上游主线；执行前仍应确认本地无待保留的工作区修改。

### 首页三评分与说明卡

- `db3914e`：无评分支撑数据时，说明卡统一为“评分数据不足”及上传支撑数据/点击圆环查看详情的简短引导，不再在首页列举 HRV、报告类型等详细缺口。
- `dba0c25`：压力、恢复、炎症三枚评分圆环的底环和进度弧线共用 `+90°` 旋转，开口由正上方移至正下方。
- `998e7ed`：修复三项均显示 `--` 但存在零散 confidence 输入时仍落入详细提示的分支；现在只要三项评分均未生成，均显示简短引导。任一评分已生成后，才展示部分缺失的详细说明。
- `74db8d8`：说明卡在内边距之后使用全宽布局，短文案不会收缩玻璃背景，左右边缘与评分圆环卡、快捷功能及 Apple 健康模块一致。
- `e1df898`：新增 DEBUG 专用“首页三评分调试”Canvas Preview。可用三个 Stepper 分别调整压力、恢复、炎症的 `0...100` 分，实时检查圆环数字、进度、置信度刻度与说明卡；Preview 只使用局部 `@State`，不读取真实账号、不请求网络、不改变正式可信评分策略。

### 验证与当前边界

- PR #5 的 Backend full regression、Regression contracts and change gate、iOS unit/UI/Release regression 与汇总 `quality-gate` 均已通过后合入。
- 后续评分改动分别执行了聚焦 XCTest：空评分/部分缺失状态、圆环底部开口、说明卡全宽布局、Preview 数值映射均通过；每次提交前均通过 `git diff --check` 与 `fast` 轻量门禁。
- `fast` 只证明空白、配置/脚本语法等轻量检查，不替代完整 XCTest、真机视觉、Dynamic Type、VoiceOver、HealthKit、后端可信评分或新的 TestFlight 验收。三评分正式显示仍依赖服务端版本化快照与既有可信展示策略。
...
## 2026-07-30 iOS 健康报告上传提示、历史与跨容器 OCR 修复（已随 build 21 上传）
...
- 六处 `CURRENT_PROJECT_VERSION` 已由 `20` 统一为 `21`。最终 `/usr/bin/python3 -I tools/python_test_gate.py tools` 精确通过 `82/82`、零跳过，耗时 `536.845` 秒，包含 ReleasePolicy 的 `96` 个负向变异；regression guard、fast 与 impacted 也在最终树上通过。候选随后按下节从精确 `main` 归档并上传；生产后端仍未部署，Android 未修改。
