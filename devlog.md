...
- 命名回归：登录同步与身份解析 `28/28`、报告相邻逻辑 `12/12`、报告上传完整界面 `1/1` 均通过 `.xcresult` 校验；界面确定性网络审计为零逃逸，校验器自测 `8/8`，regression guard、fast 与 impacted 轻量门禁通过。
- 本轮仅修改 iOS，未修改 Android、后端或数据库；未提交、推送、递增 build 或发布 TestFlight。真实生产 `/api/users/me`、对象存储和 OCR 可用性仍需受控环境或 TestFlight 验证。
