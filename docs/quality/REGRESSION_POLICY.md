...
- `xcodebuild test` 返回 0 不代表执行过正确测试。Unit、完整 UI、小屏 UI 和 CI 都必须生成 `.xcresult`，由 `tools/validate_xcresult.py` 对照受版本控制的精确清单校验：Unit 恰好 158 项、完整 UI 恰好 9 项、Unit/完整 UI 并集恰好 167 项，小屏专项恰好执行那 2 个已登记用例；任何缺失、额外、重复、改名、skip、fail 或 expected-failure 都失败。小屏结果还必须证明设备模型是 iPhone SE（第 3 代）。
...
- 2026-07-14 的 XAGE/upstream 预合并候选已把首次 `regression_guard validate` 红灯保存到 `implementation_audit/ios_xage_more_menu_extraction_20260714/pre_refactor_regression_guard_failure.txt`：精确 XCTest 清单缺少当前分支用例，且 `XAgeMainView.swift` 的行数、struct、sheet、full-screen presentation 和 alert 共六类检查被阻断。该失败只有在完整更多菜单拆分、精确 ID 清单更新、同类扫描和新 exact commit 的全部 mandatory gate 通过后才闭环；后续单次绿灯不能覆盖这份失败证据。
...
- GitHub `XAGE` 保护仍待本修复通过 feature PR、exact merged-SHA push CI 后安装并回读，本轮不能提前写成已保护。`main` 当前比 `XAGE` 落后 72 commits，仍是旧的 fail-open CI/无兼容 `quality-gate`；同步、CI 引导和保护 `main` 不在本次 iOS XAGE 修复范围，必须作为独立授权任务处理，不能为勾选保护而静默修改。`main` 仍未保护，因此即使本轮随后完成 XAGE 保护，要求两分支均受保护的 TestFlight release gate 仍保持 blocked。
