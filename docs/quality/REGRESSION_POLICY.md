...
- `release` / `assert-release` 仅用于冻结后的 main exact-SHA TestFlight 候选，继续保留全量精确测试、必要的 PG16 集成结果、设备 Archive、五项真实签核和同一 IPA 校验。
...
- Python 同样使用运行时精确清单，而不是最低数量：backend full 恰好 331 个 ID，tools 恰好 77 个 ID。tools 不允许 skip；backend 只允许下文列出的 3 个固定 integration placeholder，因此通过结论必须写成 `328 passed + 3 skipped`。
